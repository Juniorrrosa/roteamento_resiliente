...
| `POST` | `/rota` | Calcula rotas com origem, destino, chuva e exclusões automáticas |
...
`POST /rota` recebe `{origem, destino, chuva, alternates}`. Internamente:

1. Se `origem.endereco` (string) foi fornecido em vez de `lat/lon`, geocodifica via `/geocode`
2. Lê `alagamentos_realtime` ativos do PostGIS (filtrados pelo bbox da rota + margem)
3. Monta payload Valhalla:
   - `locations`: origem + destino
   - `exclude_locations`: pontos do CGE → implementa **b(e) = ∞** do paper
